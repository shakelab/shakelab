...
def _resolve_provider_config(
    ...
    *,
    ...
    Provider paths are intentionally left unchanged here. File-backed
    providers resolve their own paths at runtime, preserving the same
    semantics used by the ShakeMap provider.

    Parameters
    ----------
    provider_id
        Registered provider backend identifier.
    config
        Provider configuration mapping.
    base_directory
        Directory containing the ShakeLabGroundMotion configuration file.
        Reserved for future provider-specific normalization.
    """
    _ = (provider_id, base_directory)
