...
class _IMTProvider(_BaseProvider):
    ...
    check_event_id
        If True, compare the event id stored in the selected dataset with the
        requested ``ScenarioEvent.event_id``.
    acceleration_unit
    ...
    def _load_event(
        ...
        dataset_event_id = self._dataset_event_id(data)

        if (
            self.check_event_id
            and dataset_event_id is not None
            and dataset_event_id != event_id
        ):
            raise ValueError(
                "ShakeScenarioIMT event-id mismatch: requested "
                f"{event_id!r}, dataset contains "
                f"{dataset_event_id!r}."
            )
    ...
    @staticmethod
    def _dataset_event_id(
        data: Mapping[str, Any],
    ) -> Optional[str]:
        """Return the optional event identifier stored in one dataset."""
