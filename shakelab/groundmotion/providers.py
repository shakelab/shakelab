...
the engineering exposure/fragility/impact layers. Engineering-level
provider configuration and asset routing are
...
``shakemap``
    Precomputed ShakeMap products read through ``shakelab.groundmotion.usgs``.
...
The GMPE provider evaluates models through vectorized RuptureContext and
SitesContext objects created from ScenarioEvent and runtime site data.
...
class _GmpeProvider(_BaseProvider):
    ...
    The provider acts as an adapter between scenario-domain objects and the
    numerical contexts consumed by a GMPE. It extracts rupture parameters
    from ``ScenarioEvent``, obtains site parameters supplied at runtime, and
    computes point-source distance measures when they are sufficient.

    Finite-fault distances such as Rjb, Rrup, Rx and Ry0 are never silently
    approximated from epicentral or hypocentral distance. They must be
    supplied explicitly until a rupture-geometry layer is available.

    Runtime parameters
    ------------------
    rupture_parameters
        Optional mapping of rupture parameters. Values override parameters
        available from ``ScenarioEvent.mechanism`` and derived event fields.
    site_parameters
        Optional mapping of site-dependent properties, for example ``vs30``.
        Scalars are broadcast to all sites by ``SitesContext``.
    distances
        Optional mapping of precomputed distance arrays, for example ``rjb``
        or ``rrup``. User-supplied distances take precedence over distances
        that the provider can derive directly.
    """

    _POINT_SOURCE_DISTANCES = frozenset({
        "repi",
        "rhypo",
    })
    ...
    def __init__(
        ...
        **gmpe_kwargs: Any,
        ...
        from shakelab.groundmotion.gmpe.registry import create_gmpe
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        """
        Evaluate a GMPE for one event at all requested sites.

        The GMPE itself is evaluated once using vectorized site arrays.
        """
        ...
        self._validate_magnitude_type(event)

        runtime = dict(kwargs)

        rupture_parameters = self._as_mapping(
            runtime.pop("rupture_parameters", None),
            "rupture_parameters",
        )
        site_parameters = self._as_mapping(
            runtime.pop("site_parameters", None),
            "site_parameters",
        )
        distances = self._as_mapping(
            runtime.pop("distances", None),
            "distances",
        )

        if runtime:
            names = ", ".join(sorted(runtime))
            raise TypeError(
                "Unsupported GMPE provider runtime keyword(s): "
                f"{names}."
            )

        rupture = self._build_rupture_context(
            event,
            rupture_parameters,
        )
        site_context = self._build_sites_context(
            site_list,
            event,
            site_parameters,
            distances,
        ...
        self._gmpe.validate_contexts(
            rupture,
            site_context,
        )

        result = self._gmpe.ground_motion(
            imt,
            rupture,
            site_context,
        )

        if not isinstance(result, GroundMotionResult):
            raise TypeError(
                f"{self._gmpe.__class__.__name__}.ground_motion() "
                "must return GroundMotionResult."
            )

        mean_ln = self._result_array(
            result.mean,
            len(site_list),
            "mean",
        )
        ...
        if result.sigma is None:
            ...
            invalid_sigma = (
                ~np.isfinite(sigma_ln)
                | (sigma_ln < 0.0)
            )
            sigma_ln = sigma_ln.copy()
            sigma_ln[invalid_sigma] = np.nan

        with np.errstate(over="ignore", invalid="ignore"):
            values = np.exp(mean_ln)

        invalid_mean = ~np.isfinite(mean_ln)

        if np.any(invalid_mean):
            values = values.copy()
            values[invalid_mean] = np.nan

        return (
            values.astype(float).tolist(),
            sigma_ln.astype(float).tolist(),
        )

    def _build_rupture_context(
        self,
        event: ScenarioEvent,
        overrides: Mapping[str, Any],
    ) -> Any:
        """
        Build the rupture context requested by the selected GMPE.
        """
        from shakelab.groundmotion.gmpe.base import RuptureContext

        available: Dict[str, Any] = {
            "mag": float(event.magnitude),
            "hypo_depth": float(event.depth_km),
        }

        if event.mechanism is not None:
            for name, value in event.mechanism.items():
                if value is not None:
                    available[str(name)] = value
        ...
        requested = (
            self._gmpe.required_rupture_parameters
            | self._gmpe.optional_rupture_parameters
        )
        ...
        return RuptureContext(**data)

    def _build_sites_context(
        self,
        sites: Sequence[WgsPoint],
        event: ScenarioEvent,
        site_parameters: Mapping[str, Any],
        distances: Mapping[str, Any],
    ) -> Any:
        """
        Build vectorized site and distance data requested by the GMPE.
        """
        from shakelab.groundmotion.gmpe.base import SitesContext

        requested_site = (
            self._gmpe.required_site_parameters
            | self._gmpe.optional_site_parameters
        )

        data: Dict[str, Any] = {
            name: site_parameters[name]
            for name in requested_site
            if name in site_parameters
            and site_parameters[name] is not None
        }

        required_distances = self._gmpe.required_distances
        optional_distances = self._gmpe.optional_distances

        for metric in required_distances | optional_distances:
            if metric in distances and distances[metric] is not None:
                data[metric] = distances[metric]
                continue

            if metric in self._POINT_SOURCE_DISTANCES:
                data[metric] = self._compute_distance_array_km(
                    event,
                    sites,
                    metric,
                )
            ...
            if metric in required_distances:
                raise ValueError(
                    f"{self._gmpe.__class__.__name__} requires "
                    f"distance {metric!r}, which cannot be derived "
                    "from the point-source ScenarioEvent. Supply it "
                    "through the 'distances' runtime mapping or provide "
                    "a rupture-geometry implementation."
                )
    ...
    def _validate_magnitude_type(
        self,
        event: ScenarioEvent,
    ) -> None:
        """
        Check event magnitude metadata when both sides define a type.
        """
        required = getattr(
            self._gmpe,
            "MAGNITUDE_TYPE",
        ...
        supplied = event.magnitude_type

        if required is None or supplied is None:
            return

        required_key = str(required).strip().upper()
        supplied_key = str(supplied).strip().upper()

        if required_key != supplied_key:
            raise ValueError(
                f"{self._gmpe.__class__.__name__} requires magnitude "
                f"type {required!r}, but ScenarioEvent provides "
                f"{supplied!r}."
            )

    def _compute_distance_array_km(
        self,
        event: ScenarioEvent,
        sites: Sequence[WgsPoint],
        metric: str,
        ...
        """
        Compute a point-source distance measure for all sites.
        """
        values = [
            self._compute_distance_km(
                event,
                site,
                metric,
    ...
    def _compute_distance_km(
        ...
        """
        Compute one point-source distance in kilometres.
        """
        if metric == "repi":
            return (
                float(
        ...
        raise ValueError(
            f"Unsupported point-source distance metric: {metric!r}."
        )
    ...
    def _validate_sites(
        ...
        """
        Validate and materialize the sequence of evaluation sites.
        """
        try:
            site_list = list(sites)
        except TypeError as exc:
            raise TypeError(
                "sites must be a sequence of WgsPoint objects."
            ) from exc
    ...
    def _as_mapping(
        ...
        """
        Normalize an optional runtime mapping.
        """
        if value is None:
            return {}
    ...
    def _result_array(
        ...
        """
        Normalize one GMPE result component to one value per site.
        """
        array = np.asarray(
            value,
            dtype=float,
        ...
        if array.ndim != 1:
            raise ValueError(
                f"GroundMotionResult.{name} must be scalar or "
                "one-dimensional."
            )

        if array.size != size:
            raise ValueError(
                f"GroundMotionResult.{name} has length {array.size}, "
                f"expected {size}."
            )
...
class _ShakeMapProvider(_BaseProvider):
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        """
        from shakelab.groundmotion.usgs import ShakeMapEvent
