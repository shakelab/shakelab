...
the engineering exposure/fragility/impact layers. Engineering-level provider configuration and asset routing are implemented in
``shakelab.engineering.gmdata.associator``.
...
class _IMTProvider(_BaseProvider):
    """
    Ground-motion provider backed by a ShakeScenarioIMT JSON file.

    Parameters
    ----------
    path
        Path to the ShakeScenarioIMT JSON file.
    check_event_id
        If True, compare the dataset event id with ``ScenarioEvent.event_id``
        when both are available.
    acceleration_unit
        Output unit for PGA and SA. Supported values are ``"g"`` and
        ``"m/s2"``.
    velocity_unit
    ...
    period_tolerance
        ...
        against the period vector stored in the dataset.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.path.exists():
            raise FileNotFoundError(
                f"ShakeScenarioIMT file not found: {self.path}"
            )

        if not self.path.is_file():
            raise ValueError(
                f"ShakeScenarioIMT path is not a file: {self.path}"
            )

        self.check_event_id = bool(check_event_id)
        self.acceleration_unit = _normalize_acceleration_unit(
            acceleration_unit
        )
        ...
        self._event_id = self._dataset_event_id()
        self._imt_definitions = self._build_imt_definitions()
        self._stations = self._build_station_index()
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        """
        Evaluate one precomputed IMT for the assigned station.

        Parameters
        ----------
        imt
            Requested IMT. Supported forms include ``PGA``, ``PGV`` and
            ``SA(period)``, for example ``SA(0.3)``.
        sites
            Site sequence required by the common provider interface. Spatial
            coordinates are not used by this provider because the station is
            selected explicitly by ``station_code``.
        event
            Scenario event. Its event id is optionally cross-checked against
            the IMT dataset.
        **kwargs
            Must contain ``station_code``.

        Returns
        -------
        values, sigma_ln
            One deterministic IMT value and zero logarithmic uncertainty for
            each requested site.
        """
    ...
    def _load(self) -> Dict[str, Any]:
        """Load and validate the top-level IMT document."""
        try:
            data = json.loads(
            ...
            raise ValueError(
                f"Cannot read ShakeScenarioIMT file: {self.path}"
            ) from exc
        except json.JSONDecodeError as exc:
            raise ValueError(
                f"Invalid ShakeScenarioIMT JSON file: {self.path}"
            ) from exc
        ...
        if data.get("type") != IMT_EXPECTED_TYPE:
            raise ValueError(
                ...
                f"{data.get('type')!r}; expected {IMT_EXPECTED_TYPE!r}."
            )
    ...
    def _dataset_event_id(self) -> str | None:
        """Return the optional event id stored in the dataset."""
    ...
    def _build_imt_definitions(self) -> Dict[str, Dict[str, Any]]:
        """Validate and normalize IMT definitions."""
        definitions: Dict[str, Dict[str, Any]] = {}

        for key, value in self._data["imts"].items():
    ...
    def _build_station_index(self) -> Dict[str, Dict[str, Any]]:
        """Build an O(1) station-code lookup index."""
        index: Dict[str, Dict[str, Any]] = {}

        for position, station in enumerate(self._data["stations"]):
    ...
    def _check_event(self, event: ScenarioEvent) -> None:
        """Cross-check event identifiers when possible."""
        if not self.check_event_id:
            return

        if self._event_id is None:
            return

        runtime_id = event.event_id
        if runtime_id is None:
            return

        runtime_id = str(runtime_id).strip()

        if runtime_id and runtime_id != self._event_id:
            raise ValueError(
                "ShakeScenarioIMT event-id mismatch: requested "
                f"{runtime_id!r}, dataset contains "
                f"{self._event_id!r}."
            )

    def _station(self, station_code: Any) -> Dict[str, Any]:
        """Return station data for one assignment station code."""
    ...
    def _station_imt_value(
        ...
        """Extract and convert one station IMT value."""
        ...
        if normalized in {"PGA", "PGV"}:
            ...
            definition = self._imt_definition(normalized)
        ...
        if normalized == "SA":
            definition = self._imt_definition("SA")
    ...
    def _imt_definition(
        ...
        try:
            return self._imt_definitions[imt]
        except KeyError as exc:
            raise ValueError(
    ...
    def _period_index(
        ...
        """Return the matching SA period index."""
        best_index = None
        best_delta = None
    ...
    def _convert_acceleration(
        ...
        source = _normalize_acceleration_unit(input_unit)
